...
<div class="relative h-56 overflow-hidden rounded-lg md:h-96">
  ...
  <div class="{{ $index === 0 ? '' : 'hidden' }} duration-700 ease-in-out" data-carousel-item>
      <img src="{{ asset('storage/' . $carousel->image) }}" class="absolute block w-full -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2" alt="Carousel Image {{ $index + 1 }}">
  </div>
  @endforeach
</div>
...
<section class="bg-white">
    <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16">
        ...
        <div class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0">
            <a href="/register" class="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg  bg-primary focus:ring-4">
                Ayo Pesan Sekarang
                <svg class="w-3.5 h-3.5 ms-2 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 10">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M1 5h12m0 0L9 1m4 4L9 9"/>
                </svg>
            </a>
            <a href="/catalog" class="py-3 px-5 sm:ms-4 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-green-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
                Lihat Katalog Hewan
            </a>  
        </div>
    </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 px-4 max-w-7xl mx-auto mb-12">
        ...
        <div class="bg-white relative  rounded-2xl shadow-xs transition-transform hover:shadow-sm hover:-translate-y-1 duration-300">
          ...
          <div class="p-4">
              ...
              <p class="text-black font-bold text-lg mb-2">{{ 'Rp ' . number_format($product->price, 0, ',', '.') }}</p>
